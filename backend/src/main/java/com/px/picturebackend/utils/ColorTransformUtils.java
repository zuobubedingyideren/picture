...
public class ColorTransformUtils {
    ...
    /**
     * 获取标准颜色（将腾讯云COS数据万象返回的RGB色值转为标准的六位十六进制格式）
     * 腾讯云返回格式：0xRRGGBB，但可能缺少前导零，需要补全为完整六位RGB格式
     *
    ...
    public static String getStandardColor(String color) {
        if (color == null || color.trim().isEmpty()) {
            throw new IllegalArgumentException("颜色值不能为null或空字符串");
        }
        ...
        if (!HEX_COLOR_PATTERN.matcher(trimmedColor).matches()) {
            throw new IllegalArgumentException("无效的颜色格式，期望格式为0xRRGGBB，实际输入：" + color);
        }
     ...
     *
     * @param color 腾讯云返回的十六进制颜色代码（如：0x736246、0x80e0等）
     * @return RGB数组，格式为[R, G, B]，每个值范围为0-255
     * @throws IllegalArgumentException 当输入格式无效时抛出异常
     */
     ...
     * 验证腾讯云返回的颜色格式是否有效
     *
     * @param color 待验证的颜色字符串（如：0x736246、0x80e0等）
     ...
     * 将标准十六进制颜色代码转换为腾讯云格式
     *
     * @param standardColor 标准十六进制颜色代码（如：#736246、#36246等）
