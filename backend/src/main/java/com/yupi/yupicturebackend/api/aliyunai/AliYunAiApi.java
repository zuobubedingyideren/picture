...
/**
 * packageName: com.yupi.yupicturebackend.api.aliyunai
 *
 * @author: idpeng
 * @version: 1.0
 * @className: AliYunAiApi
 * @date: 2025/8/24 16:18
 * @description: API 调用类，通‌过 Hutool 的 HTTP 请求工具类来调用阿里云‌百炼的 API
 */
@Slf4j
@Component
    ...
    public CreateOutPaintingTaskResponse createOutPaintingTask(CreateOutPaintingTaskRequest createOutPaintingTaskRequest) {
        ...
        try (HttpResponse httpResponse = httpRequest.execute()) {
            ...
            if (!httpResponse.isOk()) {
                log.error("请求失败，状态码: {}，响应内容: {}", httpResponse.getStatus(), responseBody);
                // 根据HTTP状态码提供更详细的错误信息
                String errorMsg = switch (httpResponse.getStatus()) {
                    case 400 -> "请求参数错误，请检查请求体格式和参数值";
