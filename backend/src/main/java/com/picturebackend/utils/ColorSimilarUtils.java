...
public class ColorSimilarUtils {
     ...
     * @param color1 第一个颜色对�?
     * @param color2 第二个颜色对�?
     * @return 相似度评分（0�?之间�?为完全相同，0为完全不同）
     */
     ...
     *
     * @param hexColor1 第一个颜色的十六进制代码（如 "#FF0000" �?"FF0000"�?
     * @param hexColor2 第二个颜色的十六进制代码（如 "#FE0101" �?"FE0101"�?
     * @return 相似度评分（0�?之间�?为完全相同，0为完全不同）
     */
     ...
     * 根据RGB值计算颜色相似度
     *
     * @param r1 第一个颜色的红色分量�?-255�?
     * @param g1 第一个颜色的绿色分量�?-255�?
     * @param b1 第一个颜色的蓝色分量�?-255�?
     * @param r2 第二个颜色的红色分量�?-255�?
     * @param g2 第二个颜色的绿色分量�?-255�?
     * @param b2 第二个颜色的蓝色分量�?-255�?
     * @return 相似度评分（0�?之间�?为完全相同，0为完全不同）
     */
    ...
    private static Color parseHexColor(String hexColor) {
        ...
        if (!HEX_PATTERN.matcher(cleanHex).matches()) {
            throw new IllegalArgumentException("无效的十六进制颜色格�? " + hexColor);
        }
    ...
    private static void validateRgbValue(int value, String paramName) {
        if (value < 0 || value > 255) {
            throw new IllegalArgumentException(paramName + "必须�?�?55之间，当前�? " + value);
        }
    }
    ...
    public static void main(String[] args) {
        System.out.println("=== 颜色相似度计算工具测�?===");
        
        // 测试颜色
        Color red1 = new Color(255, 0, 0);     // 纯红�?
        Color red2 = new Color(254, 1, 1);     // 接近红色
        Color blue = new Color(0, 0, 255);     // 纯蓝�?
        ...
        System.out.printf("红色vs接近红色 (阈�?.9): %s\n", similar1 ? "相似" : "不相�?);
        System.out.printf("红色vs蓝色 (阈�?.9): %s\n", similar2 ? "相似" : "不相�?);
